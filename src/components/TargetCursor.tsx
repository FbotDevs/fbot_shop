...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  // portal node so cursor is mounted directly on document.body and avoids stacking contexts
  const portalNodeRef = useRef<HTMLDivElement | null>(null);
  useEffect(() => {
    const node = document.createElement('div');
    node.setAttribute('data-target-cursor-portal', '');
  ...
  useEffect(() => {
    ...
    cornersRef.current = cursor.querySelectorAll<HTMLDivElement>('.target-cursor-corner') || null;
    ...
    const createSpinTimeline = () => {
      ...
      spinTl.current.to(cursor, { rotation: '+=360', duration: spinDuration, ease: 'none' });
    };
  ...
  if (portalNodeRef.current) {
    return createPortal(cursorElement, portalNodeRef.current);
  }
